module github.com/nickwells/netsetter.mod

go 1.21

toolchain go1.21.0

require github.com/nickwells/param.mod/v6 v6.0.0

require (
	github.com/nickwells/check.mod/v2 v2.1.12 // indirect
	github.com/nickwells/mathutil.mod/v2 v2.2.2 // indirect
	github.com/nickwells/tempus.mod v1.0.4 // indirect
	golang.org/x/exp v0.0.0-20230905200255-921286631fa9 // indirect
)

require (
	github.com/nickwells/english.mod v1.1.5 // indirect
	github.com/nickwells/errutil.mod v1.2.13 // indirect
	github.com/nickwells/filecheck.mod v1.1.22 // indirect
	github.com/nickwells/fileparse.mod v1.1.31 // indirect
	github.com/nickwells/location.mod v1.2.27 // indirect
	github.com/nickwells/pager.mod v1.0.11 // indirect
	github.com/nickwells/strdist.mod v1.1.12 // indirect
	github.com/nickwells/twrap.mod v1.5.3 // indirect
	golang.org/x/sys v0.12.0 // indirect
	golang.org/x/term v0.12.0 // indirect
)
